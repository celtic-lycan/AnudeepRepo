...
import pandas as pd
import numpy as np
from ProjectSQL.db_connection import connect_to_database, fetch_data  
...
if customer_data is None or product_data is None or order_data is None:
    ...
    # Merge with customer_data to get the customer names
    frequent_customers = pd.merge(frequent_customers, customer_data[['customer_id', 'name']], on='customer_id')

    # Create a bar chart for the top 10 most frequent customers
    top_10_customers = frequent_customers.head(10)

    plt.figure(figsize=(10, 6))
    plt.bar(top_10_customers['name'], top_10_customers['order_count'], color='salmon')
